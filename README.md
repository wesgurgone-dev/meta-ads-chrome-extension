...
The panel is a liquid-glass pane: a translucent surface that blurs and saturates the page behind it, with the dot grid riding on the glass, a specular sheen down the top-left, and an inner highlight edge. Every surface above it (rail, tabs, header, footer, cards) is translucent so the whole thing reads as one material rather than stacked panels. Saved ads render as mini cards, the dashboard's card anatomy at panel scale: creative, status and format badges, advertiser, longevity, list colour, and a full-width Download; the creative and the advertiser name open the ad in the Ad Library.
